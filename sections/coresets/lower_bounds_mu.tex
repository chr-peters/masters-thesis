...
We will conclude this section by presenting a simple way to obtain
a tight upper bound of $\mu$ for a given dataset.
But in order to do that, we first need to prove a simple
property of $\mu$ in the following lemma.
...
\begin{theorem}
    Let $\mathcal{D}$ be a $d$-dimensional dataset with
    weight vector $w \in \mathbb{R}^n_{>0}$ for which the maximum
    likelihood estimate $\hat\beta$ of the probit model exists
    and let $I_\beta^+ = \{i \in [n]:\ w_i z_i^T \beta > 0 \}$
    and $I_\beta^- = \{i \in [n]:\ w_i z_i^T \beta < 0 \}$.
    Then,
    \begin{equation*}
        \mu_w(\mathcal{D}) \leq
        \frac{\sum_{i \in I_{\hat\beta}^+} w_i (z_i^T \hat\beta)^2}
        {\sum_{i \in I_{\hat\beta}^-} w_i (z_i^T \hat\beta)^2}.
    \end{equation*}
\end{theorem}
\begin{proof}
    TODO.
