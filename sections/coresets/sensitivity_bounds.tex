...
In order to find the upper bounds on the sensitivities,
it makes sense to closer examine the function
$g = \ln \left(\frac{1}{1 - \Phi(x)}\right)$.
The following two lemmas will derive upper and lower bounds
on $g$, which will later be useful when bounding the sensitivities.
...
To recap, we have shown that $g(x) \geq \frac{1}{2} x^2$ for all
