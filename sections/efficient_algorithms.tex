...
\begin{algorithm}
    ...
    \For{$i = 1,...,n$}{
        ...
        $\tilde{Z}_j = \tilde{Z}_j + l \cdot \sqrt{w_i} x_i$ \tcc*[r]{update the $j$'th row of $\tilde{Z}$}
    }
    ...
    \For{$i = 1, ..., n$}{
        ...
        \tcc*[r]{rounding to control VC dimension}
        \For{$j = 1, ..., k$} {
            Feed $(x_i, y_i)$ with sampling weight $s_i$ to $S_j$ \;
            \If{$S_j$ \textup{samples} $(x_i, y_i)$}{
                $u_j = \frac{w_j}{s_i k}$\tcc*[r]{unnormalized weights}
            }
        }
    }
