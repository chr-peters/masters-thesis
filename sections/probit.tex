...
It is a statistical method for analyzing data sets where the response
is binary, but before we can go any further, we first have to
make clear what we mean by that.
We do this by giving a formal definition of what we will refer to
as a data set in the remainder of this work.
...
We can use this definition of a data set
(we will omit the term binary from now on since we will only be dealing
with binary datasets), to describe a whole range of possible scenarios
that we would like to subject to statistical analysis.
Perhaps the $x_i$ could represent information on a patient, such as
...
relationship between quantities that we believe have some
explanatory value (modeled by the $x_i$ in the data set)
and some related outcomes (modeled by the $y_i$).
...
but instead of starting with its GLM formulation, we introduce
